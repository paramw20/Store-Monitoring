...
# create a mutex to protect access to the database
    ...
    with app.app_context():
        ...
        with database_mutex:
            for stores_df in stores_csv:
                ...
                stores_df['timestamp_utc'] = pd.to_datetime(stores_df['timestamp_utc'], format='mixed')
        ...
        if last_status == "open":
            ...
            for j in range((status_changes[i - 1].timestamp_utc.date() - start_date).days, (status_change.timestamp_utc.date() - start_date).days):
                ...
                if business_hours[date][1] < business_hours[date][0]:
                    ...
                    downtime += max(timedelta(), business_hours[date][1] - business_hours[date][0])
            uptime += time_diff
        else:
